...
- **`/request/reimburse/approvals`** (new page) and `GET /api/request/reimburse/approvals` are the accounting queue: every claim parked at `(ManagerApproved, ACCOUNT)`, gated on the `approvalQueue` grant above. Reached from a new `REQUEST_CARDS` entry, deliberately **not** `devHostOnly` — the same argument the AP-4 settings card already carries, verbatim: the people who work this queue are not on localhost. One payment-date control covers the whole selection rather than a picker per row, and approve loops the existing per-request route once per selected id rather than a new bulk endpoint, for the reason AP-17's bulk payout-date control already gives — every guard on the single-row route (the roster check, the `(Status, CurrentStepCode)` claim) is per request, and a bulk endpoint would have to re-implement all of them and could only get one wrong.
