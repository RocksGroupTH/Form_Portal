...
- **AP-3.2's G/L categories are split in two since 2026-09-14: the category is shared, the RULE is per company** (migration 151). `AccClearAdvanceGl` keeps the account number and its Thai and English names — one row, every company — while `AccClearAdvanceGlCompany` holds `(Company, GlAccountNo) → DimensionType, IsActive`. Duplicating the names per company would be four places for one Thai name to drift; the dimension and the on/off switch genuinely differ, which is what was asked.
  ...
  - **`DimensionType` still has exactly three values and deliberately no fourth.** The screen shows two checkboxes — Branch and Employee — and `Both` is both of them ticked. There is no value meaning "neither", so **unticking the last box is refused**: a category nobody may charge is `IsActive = 0`, which makes "at least one dimension" a property of the storage rather than a check somebody has to remember. `gl-dimension.ts` is the pure mapping, and `ใช้งาน` is disabled while a company has no rule at all.
