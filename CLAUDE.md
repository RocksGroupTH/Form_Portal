...
- **Pages:** `/request/reimburse/admin` (the hub — see below), `/request/reimburse` (fill/resume draft), `/request/reimburse/[id]` (detail + timeline), `/request/reimburse/approvals` (two tabs: the accounting queue and Interface ERP), `/request/reimburse/settings` — **five** tabs in the order `REIMBURSE_SETTINGS_TAB_ORDER` declares, configuration before rosters: **แบรนด์ที่เบิกได้**, **ระเบียบการจ่าย**, **Interface ERP** (added 2026-09-09), **G/L Account** (added 2026-09-14 — AP-3's own tab for the same rows is still called บัญชีตาม BU, and the tab KEY here is still `buGlMap`, so bookmarked `?tab=` links keep working), and **สิทธิ์เข้าถึง** last. **The separate ผู้อนุมัติบัญชี tab was removed on 2026-09-10** and its route deleted with it; the approver pool is now edited on สิทธิ์เข้าถึง — see below. The page still *opens* on สิทธิ์เข้าถึง rather than the first tab, for the reason it used to open on ผู้อนุมัติบัญชี: an empty approver pool stops every claim dead and is the one thing that has to be set before AP-4 works at all
  - **G/L Account edits AP-3's ROWS, through AP-4's OWN ROUTE, and both halves of that are deliberate** (2026-09-14). `AccClrBuGlMap` (BU → G/L) and `AccClrBranchGlMap` (branch → G/L, checked first, being the more specific) carry **no `FormCode` column**: which account an expense posts to is a fact about a shop — an owned store books it to the coded account, a franchised or managed one to a receivable because the money is charged back — and that does not change with the form the expense arrived on. So there is one set of rules and AP-4 edits AP-3's. **The route is still AP-4's own** (`/api/request/reimburse/settings/bu-gl-map`), because `ROUTE_RULES` classifies by path: `/api/request/clear-advance` resolves **AP-3's** environment, and these rows are read through `getAccPool()` — a tester with AP-4 in UAT and AP-3 in production would otherwise edit production's rules from AP-4's settings page and see none of their own. **The two forms have different SCREENS over those same rows, and that is the decision** (user, 2026-09-14): AP-3's `ClrBuGlMapSettings` is untouched — a row per BU with a box to type an account into — while AP-4's `ReimburseBuGlSettings` groups the rules **by the account they point at**, which is the question an accountant actually has ("which shops book to this receivable?"). Three properties of that shape fall out of the schema rather than out of taste, and the screen says each of them out loud: a **group is not stored** (it is the set of accounts some rule names, so an empty one lives on screen only until its first member and vanishes when its last is removed); **adding is an upsert, so adding IS moving** — the picker offers every BU and branch and labels the ones already ruled with the account they sit in; and **ยังไม่ตั้งกฎ lists BUs only**, because a Company carries a handful of BUs and several hundred branches (measured 2026-09-14: PCTH has 8 and 225). Every list is a real one — BUs from the synced Locations, branches from the BRANCH dimension, accounts from `ErpAccounts`. **The account list is deliberately NOT filtered to expense accounts**: `listExpenseAccounts` would exclude `110721001`, which is what every rule in use today points at. A whole group can be deleted, which is **N writes rather than one** — there is no group row to delete, the same fact that makes an empty group unsaveable — so it stops at the first refusal and names the member it stopped on rather than leaving a half-deleted group behind one message. AP-4's screen renders a Thai line saying the rules are shared with AP-3. `requireRole` on both handlers and ungrantable, for one step more than `erpInterface`'s reason: a grant here would be a grant over **another form's** posting rules.
