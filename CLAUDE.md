...
- **Each form also has a MESSAGE, the box at the top of the form, editable without a deploy** (migration 164, `Fast_Core.dbo.FormMessage`, 2026-09-25). Beside `FormOwner`, and for a reason stronger than that one's: `FormCode` is the primary key and there is **no identity column at all**, so none of the dual-write lockstep hazards this file documents at length for `AccTravelPerDiemCountry` and `AccFormBrand` can apply here — there is no counter to drift. **One physical copy. Not dual-written, not in `MASTER_TABLES`**: `npm run check:alignment` must still read **30**, and 31 means this was wrongly added to that list.
  ...
  - **The Message tab is admin-only on all five forms, and the reason is where the grant would be stored, not what it would let somebody do.** A message grants nothing — no approval, no posting target, no read of anything else — so read this as a fact about ACC Portal's code, not a judgement that the capability is dangerous. All four settings-tab tables this app grants individual tabs through — `AccApproverSettingsTab` (AP-1), `AccBookingApproverTab` (AP-17), `AccReimburseAccessTab` (AP-4), `AccAdvClrAccessTab` (AP-2/AP-3) — are the **same rows** ACC Portal reads and writes (see "Shared with Rocks Fast — and with ACC Portal" above). That sibling's own save deletes every tab row for an approver and re-inserts only the keys its own list recognises (`ACC_Portal/src/lib/acc/approver-settings-tabs.ts:47-56`; `booking-approver-tabs.ts:93-102` is the AP-17 twin) — a key this app invented and that app has never heard of is not preserved, it is silently dropped on that app's very next save, with no error on either side. That is the *identical* failure this file already records as having happened for real, for AP-17's booking-queue and accounting-sign-off menu ticks (see "AP-17's menu grants are COLUMNS on the roster row… This replaced a second, private answer to the same question" above) — a roster edit made here, deleted there, discovered only when somebody asked why the two apps disagreed. `"messages"` (AP-1, AP-17, AP-4) and `"advanceMessages"` / `"clearMessages"` (AP-2, AP-3 — two keys rather than one shared key, for the same per-form-route reason Interface ERP is split into `advanceErpInterface` / `clearErpInterface`) are therefore excluded from every `Grantable*TabKey` union and every `GRANTABLE_*` filter, on all five forms, even though nothing about the message itself calls for it. **The remedy, if this is ever revisited, is to add the key to both applications in the same change** — never to widen the type here alone, which is exactly the mistake that would reopen the AP-17 menu-ticks defect one level up.
