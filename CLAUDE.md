...
| **Rocks_Portal_Form_UAT** (`UatTesterPerDiem`) | `getUatFormPool()` | A per-tester per-diem override read only in UAT (migrations 139/140, moved from `Fast_Core` 2026-09-07). `UatTester` itself did **not** move and stays in `Fast_Core` — see "Parallel Production and UAT" below for why the two tables were split. **One physical copy, no dual-write, not in `MASTER_TABLES`.** A literal pool, never `getFormPool()`/`getAccPool()`/`getCorePool()`/`getProductionFormPool()`. No synonym: nothing outside this app names the table |
...
- **In UAT, a tester's per diem can come from their own configured rate instead
  ...
  **never `getCorePool()`, `getFormPool()` or `getAccPool()`**, and no synonym
  ...
  stays in `Fast_Core` — see "Parallel Production and UAT" above. Design:
