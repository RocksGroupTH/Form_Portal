...
- **An expense row asks for its receipt before it asks for money.** `ExpenseRow` (`src/features/accounting/components/ExpenseRows.tsx`) shows the attach button alone until a file is on the row; attaching reveals จำนวนเงิน and reads the image for a total to prefill, editable. That makes the server's long-standing "an amount needs a receipt" rule (`validateForSubmit`) mostly unreachable rather than replacing it — **both checks stay**, because a draft written before this still holds an amount with no image, and that is also why the field is shown regardless once `amount > 0`. It never overwrites a figure already there, including one typed while the read was running.
- **The read is Claude, not OCR, and it costs money per attach.** `POST /api/request/accounting/receipt-amount` sends the image to `claude-sonnet-5` and gets a typed `{ amount }` back through `messages.parse` + `zodOutputFormat` — no text parsing anywhere. A first cut used tesseract.js plus a regex over its output; it was replaced on 2026-08-24 because Thai thermal receipts defeat it, and its parser was deleted rather than left as dead code. Four things hold that endpoint together, and none is optional:
  - **`requireAuth()`, then a per-user rate limit, before the body is even read** — 40 calls per 10 minutes (`consumeRateLimit`, `src/lib/rate-limit.ts`; the pure `decideRateLimit` is unit-tested). Invisible while filling a real claim, and a stuck retry loop hits it in seconds. It is an **in-process `Map`**, so a second Node instance counts separately — the same caveat `auth.ts`'s jwt cache carries.
  - **`checkAttachment({ allowedKinds: ["image"] })`** — the AP-1 upload's own magic-byte allowlist, so a renamed `.exe` is refused rather than paid for. Then narrowed again to the four media types the Messages API takes: `allowedKinds: ["image"]` also admits HEIC, which the client never sends but a direct caller could.
  - **`sanitizeReceiptAmount`** (`src/features/accounting/lib/receipt-amount.ts` — same path as the deleted parser, new job) is the last gate: it refuses 0, negatives, non-finite values and anything above `MAX_RECEIPT_AMOUNT` (1,000,000฿). A model can misread, and the tax id printed on every Thai receipt is the number most likely to come back as one. **A blank editable field beats a wrong figure on a form about to be submitted**, which is why every failure path returns null rather than a guess.
  - **The read never gates the field.** Once a receipt is attached the amount input is on screen and editable, whatever the read is doing; `readNote` drives a line of copy *beside* it, never in place of it. The first cut swapped the input out for a spinner and so locked the requester out of their own field for as long as the call took — up to the 30s timeout — which is the opposite of the fallback this is supposed to have. Re-attaching the image is the retry; the note clears itself the moment the field holds a figure, however it got there.
  - **The "still mounted?" ref is set on mount, not only cleared on unmount** (`aliveRef`, `ExpenseRows.tsx`). `useEffect(() => () => { ref.current = false }, [])` looks right and is not: `reactStrictMode` is unset in `next.config.mjs`, which means **on**, so development runs effects mount → cleanup → mount and the ref stays false for the rest of the component's life. Every read then returned early — the note sat on "กำลังอ่านยอด" forever and no amount was ever filled. The other async paths in this app use a local `let cancelled = false` **inside** the effect, which is fresh per run and has no such trap; prefer that shape.
  - **A failure says which failure.** `readReceiptAmount` returns a `ReceiptFailure` alongside the null — `not-found` (the call worked, this image has no trustworthy total), `unavailable` (503 from `statusForVisionError`: a missing or revoked key, only an operator can fix it) or `error` (upstream trouble, offline) — and `RECEIPT_FAILURE_TEXT` is a `Record` over that union, so adding a kind without copy is a compile error. One line for all three said "อ่านยอดจากใบเสร็จไม่สำเร็จ", which reads as *your receipt is no good* even when the fault is ours; the same wording bug bit AP-17's ID-card dialog on the same day, from the same 401.
  - **The client downscales before posting** (`read-receipt-amount.ts` → `toDownscaledCanvas` at 1600px → JPEG). Not cosmetic: image tokens scale with area and the call is billed. Re-encoding is also what guarantees a media type the API accepts whatever the phone produced.
  ...
  - `ROUTE_RULES` needs no entry: the `/api/request/accounting` prefix already classifies `AP-1`, and the route reads no database at all.
...
- **The "is this a national ID card?" check is Claude, and the heuristic it replaced never worked.** `POST /api/request/travel-booking/id-card-check` sends the image to `claude-sonnet-5` for a typed `{ isIdCard, reason }`. Until 2026-08-24 the check ran tesseract in the browser and passed an image on **either** a 13-digit run **or** a Thai ID keyword — and a Thai **tax id is also exactly 13 digits**, printed on every ใบกำกับภาษี, so a receipt verified as a national ID card and got the green "ตรวจสอบแล้วเป็นบัตรประชาชน" badge. Its digit pattern allowed a space or a dot between digits too, so one line of prices (`199.00 249.00 30.00`) matched as well — measured, not inferred. **No regex over OCR text can separate a tax id from a national ID number**; do not try to bring one back.
  ...
  - **It fails closed, and that is a decision with a stated cost.** Nothing is attached without a verdict of `true`: no key, rate limited, upstream down, offline and "this is not a card" all refuse the file identically. **So while the check cannot run, AP-17 cannot be filed at all** — the card is required to submit. Fail-open was built first and rejected on 2026-08-24, the trade named and accepted: a card image nobody verified is exactly what the old heuristic produced, and stopping that was the point. A `null` answer from the model is likewise never a pass. `looksLikeThaiIdCard` (`features/travel-booking/lib/idcard-check.ts`) still marks the no-verdict case `unavailable`, but only so the copy can name the right remedy — *wait* (429), *tell IT* (503), *retry*, or *attach a different photo* are not interchangeable advice once a failure blocks the requester. The green "ตรวจสอบแล้วเป็นบัตรประชาชน" badge is unconditional again, and truthful, because nothing becomes a pending file without passing.
  - **The two rate-limit buckets are keyed per purpose (`id-card-check:` / `receipt-amount:`), and fail-closed is why that matters.** Sharing one bucket would let a requester burn their allowance on AP-1 receipt reads and then be unable to attach an ID card at all.
  - **An invalid key and a missing one are the same situation, and neither is "try again".** `statusForVisionError` (`src/lib/acc/vision-error.ts`, pure and unit-tested) maps an upstream **401/403/400** to **503** and everything else to 502 — because retrying a revoked key fails identically forever, and the only remedy is an operator's. Found the hard way on 2026-08-24: a key that was present and the right length answered `401 authentication_error — API key is invalid`, and the requester was shown a dialog titled **"ไม่ใช่บัตรประชาชน"** telling them to pick a different photo. Both halves were wrong, and the dialog now reads its title and its button off `refusal.unavailable` instead of hardcoding the not-a-card wording. **When somebody reports a valid ID card being rejected, check the key before the photo.**
  - Guards, rate limit and client construction are shared with AP-1's receipt read through `src/lib/acc/vision-guard.ts` — one copy, because each step there is either a cost control or an upload guard and two copies would drift. `ROUTE_RULES` needs no entry: `/api/request/travel-booking` already classifies `AP-17`.
  - **Nothing in this app OCRs in the browser any more.** `tesseract.js` is uninstalled and `src/lib/ocr/image-text.ts` is gone; what survived is its decode-and-downscale half, now `src/lib/image/downscale.ts` (`toDownscaledCanvas`), which both image reads use before posting — the 1600px cap is a cost control, since image tokens scale with area.
- **จุดขึ้นรถ/ขึ้นเครื่อง carries a default**: ขาไป `กรุงเทพมหานคร` (`GO_DEFAULT_DEPARTURE_PLACE`, spelt as the province master spells it), ขากลับ the province being travelled to. The rule is `src/features/travel-booking/lib/departure-default.ts` — pure, unit-tested — and it fills an empty field or one still holding *its own* previous fill, never a place the requester chose. Telling those apart needs `goAppliedDeparturePlace` / `returnAppliedDeparturePlace` on `TabFormState` (UI-only; `buildSaveInput` does not send them), because somebody who types "เชียงใหม่" by hand is otherwise indistinguishable from the default that would have written it. **It is called from three handlers, not an effect** — `OrsPlaceField` commits on every keystroke, so a fill watching `departureLocations` would put the default straight back the moment somebody cleared the field. The three are picking a vehicle, choosing a province, and the ORS province auto-detect. A resumed draft is left alone (both applied-fields load as null), so an empty place on one is filled only if the province is then changed.
