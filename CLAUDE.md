...
- **Pages:** `/request/reimburse/admin` (the hub — see below), `/request/reimburse` (fill/resume draft), `/request/reimburse/[id]` (detail + timeline), `/request/reimburse/approvals` (two tabs: the accounting queue and Interface ERP), `/request/reimburse/settings` — **four** tabs in the order `REIMBURSE_SETTINGS_TAB_ORDER` declares, configuration before rosters: **แบรนด์ที่เบิกได้**, **ระเบียบการจ่าย**, **Interface ERP** (added 2026-09-09), and **สิทธิ์เข้าถึง** last. **The separate ผู้อนุมัติบัญชี tab was removed on 2026-09-10** and its route deleted with it; the approver pool is now edited on สิทธิ์เข้าถึง — see below. The page still *opens* on สิทธิ์เข้าถึง rather than the first tab, for the reason it used to open on ผู้อนุมัติบัญชี: an empty approver pool stops every claim dead and is the one thing that has to be set before AP-4 works at all
