...
- **`/request/reimburse/approvals`** and `GET /api/request/reimburse/approvals` are the accounting queue: every claim parked at `(ManagerApproved, ACCOUNT)`, gated on the `approvalQueue` grant above. **Since 2026-09-09 the page is two tabs** — this queue and the read-only Interface ERP list — and it is reached from AP-4's hub rather than from a card of its own; the card that named it directly was deleted in the merge above. `GET /api/request/reimburse/erp-queue` takes the same `approvalQueue` gate and has its own `erp-queue-route-authz-guard.test.ts`, because that one `if` is all that stands between every authenticated employee and every approved claim's number, requester, amount and payment date — and this codebase has *measured* that deleting such a gate leaves the suite green. One payment-date control covers the whole selection rather than a picker per row, and approve loops the existing per-request route once per selected id rather than a new bulk endpoint, for the reason AP-17's bulk payout-date control already gives — every guard on the single-row route (the roster check, the `(Status, CurrentStepCode)` claim) is per request, and a bulk endpoint would have to re-implement all of them and could only get one wrong.
  ...
  - **AP-1's equivalent has a fail-open that AP-4 deliberately does not reproduce.** Measured 2026-09-10: in AP-1, all-four-ticked and none-ticked both store **zero** `AccApproverInterfaceBrand` rows, and zero rows reads as **unrestricted** — so clearing a KSI-only approver's last tick silently promotes them to every brand, with the grid then showing all four ticked, which is the state the admin thought they were clearing. **AP-4 has no state where absence means "all"**: rows are always explicit, zero rows means zero brands, and zero brands means not an approver. **AP-1 was deliberately left alone** — it is nine server paths plus the ACC Portal sibling reading the same `AccApprover` rows, and nobody has measured which existing zero-row approvers are unrestricted on purpose. Do not "fix" AP-4 to match it.

    - **Measured 2026-09-24, and the grid and the server disagree about a live approver.** Of AP-1's seven active approvers, six carry explicit rows (`PCTH`, one of them `PCTH, UNO`) and **one — Plume Pasapong — carries zero**. The settings grid shows that person with **nothing ticked**, which reads as no access; `resolveApproverInterfaceAccess` reads the same zero rows as `allAccess: true` and lets them approve **every** brand. Both surfaces are behaving as written, and the value they disagree about is the same value.
    - **`toPayload` is where the ambiguity is created, and only one of its three cases is broken.** None ticked stores `[]`, all ticked stores `null`, and both land as zero rows — so *all* and *none* are the same stored state. Ticking **some** stores an explicit list and is unambiguous. So the safe way to scope somebody is to tick at least one brand and fewer than all of them; the failure is specific to an approver nobody has ticked anything for.
    - **The user chose the data fix on 2026-09-24**, not a rule change: tick the brands that approver actually looks after, which makes the grid, the `รออนุมัติโดย` tooltip and the approve button agree for them. **The trap is untouched for the next zero-row approver**, which is why it is measured and dated here rather than left as the general warning above.
