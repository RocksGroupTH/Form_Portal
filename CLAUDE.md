...
- **`Home`** (`/`) — a form catalogue: greeting and stat strip, search, "Continue where you left off" (resumable drafts and Returned requests), then the **Accounting** forms — AP-1 travel expense, AP-2 advance, AP-3 clear advance, AP-17 travel booking and AP-4 staff reimbursement, filtered to the ones available to this viewer. It is a link surface only: it creates no API of its own beyond reading `/api/form-environment` for availability. `src/features/home/HomeCatalogue.tsx`.
  - **The stat strip is eight tiles and every one of them is a link** (the user, 2026-09-24). รออนุมัติจากคุณ opens My Work; the other seven open My Requests **with the filter they counted already applied**, so the number and the page it opens are the same rows. `countHomeStats` (`src/features/home/lib/home-stats.ts`, pure and tested) counts them in one pass over `/requests/mine`, which Home was already fetching for the month count alone — **no new endpoint**. It counts with the *same predicates* My Requests filters by, which is the whole reason the link can be trusted.
  - **Three decisions worth not re-litigating.** คำขอทั้งหมด **excludes never-submitted drafts**, because `listMyRequestRows` pins `Status <> 'Draft'` and drafts cannot be shown on My Requests at all; they have their own tile. Every status tile counts **all time**, with คำขอเดือนนี้ the one exception — and it measures `submittedAt`, the field `inDateRange` filters on, so its `?from=&to=` link lands on exactly what it counted. **ร่าง / ตีกลับ has no link**, deliberately: any filter would open a shorter list than the tile promises.
  - **`Returned` is in two tiles at once**, and the strip is a set of answers rather than a partition: it is a filed request with a status of its own *and* something its owner can still edit.
...
- **`My Requests`** (`/my-request`) — the Accounting requests you submitted and their status. Form-agnostic: `listMyRequestRows` filters on ownership, not `FormCode`, so AP-4 rows appear alongside AP-1's and AP-17's, merged across both databases by `src/lib/acc/query-both.ts`. **Drafts are in it since 2026-09-24** — that query used to pin `Status <> 'Draft'`, which is what made Home's ร่าง / ตีกลับ tile unlinkable: it counted drafts and returns while no destination could show a draft. Both halves moved together, so the tile counts from these same rows (an AP-3 draft was in nobody's count before, the two drafts endpoints serving AP-1 and AP-17 alone) and its `?status=Draft,Returned` link opens exactly what it counted. A draft carries no `RequestNo` and no `SubmittedAt`, so it prints — in both columns and drops out of any วันที่ส่ง range, which is correct rather than tolerated: it has not been sent. **`listMyWorkRows` is unaffected and must stay so** — it matches on an approval row, and a draft has none, which is also why `Draft` is an option on this page's status filter and deliberately not on My Work's.
...
- **163 is `Fast_Core` only, and a deployment without it is DEGRADED rather than broken — deliberately.** `163_core_form_owner.sql` creates `dbo.FormOwner`, the people each form names as its owners. `listFormOwners` swallows **`Invalid object name`** and nothing else, so before it is applied every form prints the bare `กรณีต้องการยกเลิกติดต่อเจ้าของฟอร์ม` it printed before — the wording it has always had — and the settings column shows nobody. Any other failure rethrows, because silently printing "no owners" over a Fast_Core outage would hide the outage without helping anybody. That is why the code could ship before the migration, which is the opposite of 090/120/144/156/158's rule and is worth knowing which case you are in.
  ...
  - **Written and NOT applied as at 2026-09-24.** Applying is the user's call, as it is for every migration here.
