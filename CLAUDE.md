...
- **สิทธิ์เข้าถึง is a second roster, not a rename of ผู้อนุมัติบัญชี, and the distinction is the whole point — it survived the two tabs becoming one screen on 2026-09-10.** `AccReimburseApprover` is the pool that takes the ACCOUNT and ACCOUNT_FINAL steps — a row on it approves real money. `AccReimburseAccess` + `AccReimburseAccessTab` (migration **120**, renumbered from 106) decide who may *open* which of AP-4's settings tabs. AP-17 hangs its tab grants straight off `AccBookingApprover` because that roster only ever granted sight of a queue and a report; doing the same here would have made "may edit the payment rules" and "may approve a payment" the same tick, with no way to hand out the first alone.
  ...
  - **The สิทธิ์เข้าถึง grid lists EVERY settings tab, not only the grantable ones** (2026-09-14, user: the column group showed two of six and read as incomplete). The four that cannot be handed out render greyed, with a dash instead of a checkbox and the reason on hover — an admin looking for "who may open Interface ERP" should find the answer on this screen rather than conclude the tab is missing. `ALL_REIMBURSE_TABS` is mapped from `REIMBURSE_SETTINGS_TAB_ORDER`, so a new tab appears in the header and the body or in neither, and it changes nothing about enforcement: the saved payload is still built from `GRANTABLE_REIMBURSE_TABS`, `filterStorableReimburseKeys` still refuses to write the other four and `decideReimburseTabAccess` still refuses to open them. AP-2 / AP-3's own grid does the same through `ALL_ADV_CLR_TABS`.
  - **Three of the five tabs can never be granted.** `GRANTABLE_REIMBURSE_TABS` (`src/lib/acc/reimburse/settings-tabs.ts`, pure and unit-tested) is filtered from the page's own tab order down to `rules` and `brands`. `access` is excluded for the reason AP-1 and AP-17 both give — whoever opens it can grant themselves the rest — and `erpInterface` because it is not brand-scoped, so a scoped grant holder could set another brand's posting configuration. (Until 2026-09-10 the second exclusion was `approvers`, for the sharper reason that it edited the payment pool directly. That tab no longer exists; its job moved onto `access`, which was already ungrantable, so nothing was widened.) Neither exclusion is a database constraint: `AccReimburseAccessTab` has no CHECK on `TabKey` and is writable from more than one place, so a row naming any string can appear, and `decideReimburseTabAccess` refusing it is what makes that inert.
