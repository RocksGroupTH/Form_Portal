...
- **The form's live per-diem estimate moved with both rules above, one commit after the paragraph below was written to say it had not (2026-09-22, I2).** This paragraph said, until then: *"`useTravelBookingForm.ts`'s own `continuationFlags` is still `tabs[i - 1]`… Neither the spec nor the plan ruled on this — it is a gap, not a decision."* That was true when written and is not true at HEAD — `grep -c "tabs\[i - 1\]"` on the hook returns 0 — because the very next commit closed the gap this paragraph exists to describe, and nobody updated the paragraph itself, which is the exact class of doc drift this file's own house style exists to prevent. Left here, dated, rather than deleted, for the same reason every other self-correction in this file is kept: it says what to check before trusting the next paragraph.
  ...
  - **What still legitimately differs from the submit is staleness, not logic.** The estimate prices from whatever allowance log and country rates have already landed on the form; the authoritative amount is priced from the server's own fresh read at the moment of submit, which can differ if a rate changed in between.
