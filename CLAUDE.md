...
- **The form's live per-diem estimate has not moved with either of the two rules above** — measured 2026-09-22, and written down here because nothing on the server side shows it. `useTravelBookingForm.ts`'s own `continuationFlags` is still `tabs[i - 1]`, the previous tab of *this* form and nothing else, and its `perDiemEstimates` calls `computePerDiem` with four arguments and no `opts` even though the tab state beside it already carries `needsRoomBooking`. So a requester continuing a trip filed last week is shown a day the submit will drop, and one picking an accommodation that books no room is shown money the submit will store as ฿0. **What is stored is the server's own figure and is correct** — the draft save and the submit each re-derive it; it is the number on screen while the form is being filled that is stale. The estimate block's own comment still says it mirrors the server "exactly for day-count/continuation", which is the half most likely to mislead whoever reads it next. Neither the spec nor the plan ruled on this — it is a gap, not a decision.
