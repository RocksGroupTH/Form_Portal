...
- **The "is this a national ID card?" check is Claude, and the heuristic it replaced never worked.** `POST /api/request/travel-booking/id-card-check` sends the image to `claude-sonnet-5` for a typed `{ isIdCard, reason }`. Until 2026-08-24 the check ran tesseract in the browser and passed an image on **either** a 13-digit run **or** a Thai ID keyword — and a Thai **tax id is also exactly 13 digits**, printed on every ใบกำกับภาษี, so a receipt verified as a national ID card and got the green "ตรวจสอบแล้วเป็นบัตรประชาชน" badge. Its digit pattern allowed a space or a dot between digits too, so one line of prices (`199.00 249.00 30.00`) matched as well — measured, not inferred. **No regex over OCR text can separate a tax id from a national ID number**; do not try to bring one back.
  ...
  - **The field has invited a passport since 2026-09-21 (`แนบรูปบัตรประชาชน หรือ Passport`), and the check still refuses one.** The copy change is package A of the 2026-09-21 AP-17 batch; the verification behind it was deliberately left alone, so it still fails closed on a passport four ways over: the prompt tells the model to answer `true` only for a genuine Thai national ID card (`id-card-check/route.ts:45`), the prompt's own reject list names `พาสปอร์ต` (`:46`), `AnswerSchema.isIdCard`'s description reads "True only if the image shows a Thai national ID card" (`:34`), and `FALLBACK_REASON` is `"รูปนี้ไม่ใช่บัตรประจำตัวประชาชน"` (`:54`). Combined with "It fails closed" above, **a passport holder cannot file AP-17 at all** — the exact population the new wording invites. This is known and recorded here deliberately; widening what counts as identity proof is a policy call for the user, not a copy fix, and touches the prompt's reject list, the schema description, `FALLBACK_REASON` and the refusal dialog's title together. See `docs/superpowers/specs/2026-09-21-ap17-a-mail-copy-and-title.md`'s amendment for the same note in more detail.
