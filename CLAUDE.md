...
- **สิทธิ์เข้าถึง is a second roster, not a rename of ผู้อนุมัติบัญชี, and the distinction is the whole point — it survived the two tabs becoming one screen on 2026-09-10.** `AccReimburseApprover` is the pool that takes the ACCOUNT and ACCOUNT_FINAL steps — a row on it approves real money. `AccReimburseAccess` + `AccReimburseAccessTab` (migration **120**, renumbered from 106) decide who may *open* which of AP-4's settings tabs. AP-17 hangs its tab grants straight off `AccBookingApprover` because that roster only ever granted sight of a queue and a report; doing the same here would have made "may edit the payment rules" and "may approve a payment" the same tick, with no way to hand out the first alone.
  ...
  - **Both commissioning banners moved with the tab and count people with ≥1 brand tick, not roster rows**: zero (every claim stops at the ACCOUNT step) and exactly one (the two-person rule stalls every claim permanently at ACCOUNT_FINAL — "the one that looks fine until it is tried").
...
- **`/request/reimburse/approvals`** and `GET /api/request/reimburse/approvals` are the accounting queue: every claim parked at `(ManagerApproved, ACCOUNT)`, gated on the `approvalQueue` grant above. **Since 2026-09-09 the page is two tabs** — this queue and the read-only Interface ERP list — and it is reached from AP-4's hub rather than from a card of its own; the card that named it directly was deleted in the merge above. `GET /api/request/reimburse/erp-queue` takes the same `approvalQueue` gate and has its own `erp-queue-route-authz-guard.test.ts`, because that one `if` is all that stands between every authenticated employee and every approved claim's number, requester, amount and payment date — and this codebase has *measured* that deleting such a gate leaves the suite green. One payment-date control covers the whole selection rather than a picker per row, and approve loops the existing per-request route once per selected id rather than a new bulk endpoint, for the reason AP-17's bulk payout-date control already gives — every guard on the single-row route (the roster check, the `(Status, CurrentStepCode)` claim) is per request, and a bulk endpoint would have to re-implement all of them and could only get one wrong.
  ...
  - **The claim brand maps to a target through `AccBrandErpInterface`, and a brand that maps to nothing is out of EVERY scope.** `canActOnTarget` refuses a null target for anyone, so such a claim is actionable by nobody but an admin and appears on no queue. **`ROCKS`, which migration 092 seeds as AP-4's only claimable brand out of the box, is exactly such a brand**, so this is the *default* state on a fresh deployment rather than an edge. Both queues therefore report `unmappedBrandCount` and say so on screen, naming the fix — an admin maps the brand at ตั้งค่า → Interface ERP — rather than rendering the "nothing is pending" line they would show if there were genuinely nothing.
