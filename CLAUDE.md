...
- **No accommodation booked, no per diem** (user, 2026-09-21). `computePerDiem`'s `opts.roomBooked === false` returns `{ days: 0, total: 0, groups: [] }` however many nights the trip spans — **the cut is the booking, not the calendar**, so three nights with relatives pay nothing. The flag is the request's own persisted `AccTravelBooking.NeedsRoomBooking`, which `derive-flags.ts` derives from the selected accommodation option, and both writers read it back off the row rather than off the posted DTO — the rule every other booking flag already follows.
  ...
  - **Package E's shared room is the one exception, and it does not exist yet.** A requester picking พักห้องเดียวกับ books no room of their own and *does* get per diem, so that caller will pass `roomBooked: true` explicitly. Recorded here, and in `perdiem.ts`'s own comment, so a reader who meets this rule first does not read the exception as a bug — and so the exception is built at E's call site rather than as a branch inside `computePerDiem`.
