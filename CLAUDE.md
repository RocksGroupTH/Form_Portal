...
- **`My Requests`** (`/my-request`) — the Accounting requests you submitted and their status. Form-agnostic: `listMyRequestRows` filters on ownership, not `FormCode`, so AP-4 rows appear alongside AP-1's and AP-17's, merged across both databases by `src/lib/acc/query-both.ts`.
  - **The four summary boxes ARE the status filter** (the user, 2026-09-24), with a ring marking the live one; the chip row under the form filter is gone. A **สถานะ** multi-select sits beside **ฟอร์ม** and **แบรนด์**, offering the six words each row's own chip prints — Submitted / Pending / Complete / Rejected / Revise / Cancelled — because you filter by what you can see; the old chips grouped Submitted and Pending together and could not tell them apart. **Two controls, one value**: a box lights when the selection is *exactly* its set, so picking its statuses by hand lights it and removing any one puts it out. `src/features/accounting/lib/request-status-filter.ts` holds the sets, the vocabulary and the row-to-option mapping together for that reason — separate them and the highlight starts lying. A fifth box, **ยอดรวมที่กรอง**, sums `totalAmount` over the *filtered* rows while the four counts beside it read *every* row; both are right and the labels are what say which.
  ...
  - **The page no longer opens filtered to รออนุมัติ.** It opens on ทั้งหมด. A total over a filter nobody chose is a figure nobody can reconcile, and the boxes make the status obvious in a way the chips did not. Home's `?status=` links still land where they did — `MyRequestsPanel` seeds `statusFilter`, `dateFrom` and `dateTo` from the URL **once, at mount**; re-reading them per render would snap a filter the viewer just pressed back to whatever Home linked to.
