...
- **Pages:** `/request/reimburse/admin` (the hub — see below), `/request/reimburse` (fill/resume draft), `/request/reimburse/[id]` (detail + timeline), `/request/reimburse/approvals` (two tabs: the accounting queue and Interface ERP), `/request/reimburse/settings` — **five** tabs in the order `REIMBURSE_SETTINGS_TAB_ORDER` declares, configuration before rosters: **แบรนด์ที่เบิกได้**, **ระเบียบการจ่าย**, **Interface ERP** (added 2026-09-09), **บัญชีตาม BU** (added 2026-09-14), and **สิทธิ์เข้าถึง** last. **The separate ผู้อนุมัติบัญชี tab was removed on 2026-09-10** and its route deleted with it; the approver pool is now edited on สิทธิ์เข้าถึง — see below. The page still *opens* on สิทธิ์เข้าถึง rather than the first tab, for the reason it used to open on ผู้อนุมัติบัญชี: an empty approver pool stops every claim dead and is the one thing that has to be set before AP-4 works at all
  - **บัญชีตาม BU edits AP-3's ROWS, through AP-4's OWN ROUTE, and both halves of that are deliberate** (2026-09-14). `AccClrBuGlMap` (BU → G/L) and `AccClrBranchGlMap` (branch → G/L, checked first, being the more specific) carry **no `FormCode` column**: which account an expense posts to is a fact about a shop — an owned store books it to the coded account, a franchised or managed one to a receivable because the money is charged back — and that does not change with the form the expense arrived on. So there is one set of rules and AP-4 edits AP-3's. **The route is still AP-4's own** (`/api/request/reimburse/settings/bu-gl-map`), because `ROUTE_RULES` classifies by path: `/api/request/clear-advance` resolves **AP-3's** environment, and these rows are read through `getAccPool()` — a tester with AP-4 in UAT and AP-3 in production would otherwise edit production's rules from AP-4's settings page and see none of their own. The panel is AP-3's `ClrBuGlMapSettings` with its endpoint as a prop, not a fork, and it renders a Thai line saying the rules are shared wherever the screen is not AP-3's own. `requireRole` on both handlers and ungrantable, for one step more than `erpInterface`'s reason: a grant here would be a grant over **another form's** posting rules.
