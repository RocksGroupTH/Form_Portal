...
- **Pages:** `/request/reimburse` (fill/resume draft), `/request/reimburse/[id]` (detail + timeline), `/request/reimburse/settings` — four tabs in the order `REIMBURSE_SETTINGS_TAB_ORDER` declares: **ผู้อนุมัติบัญชี** (the approval pool), **ระเบียบการจ่าย**, **แบรนด์ที่เบิกได้**, and **สิทธิ์เข้าถึง** last
