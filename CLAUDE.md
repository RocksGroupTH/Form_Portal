  ...
  **never `getCorePool()`, `getFormPool()` or `getAccPool()`**, and no synonym
  ...
  - **Every stored rate counts, and the effective date is the only selector —
    ...
    **It is a one-way door** — a tester who has ever had a rate stored can never
    be returned to HR pricing, and `null` is now reachable only from an empty
    input, which is what keeps the never-return-`[]` invariant load-bearing
  ...
  - **The near-identical twin keeps its flag and must.** `AccTravelPerDiemCountry`
    ...
    `per-diem` — and *its* pricing genuinely does filter `IsActive = 1`. `grep
    "The soft delete"` returns four hits and `grep "including inactive ones"`
    two. Edit either table's code by exact path, never by grepping the field.
