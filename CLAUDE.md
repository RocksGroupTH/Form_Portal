...
|----------|------|---------|
| **Fast_Core** | `getCorePool()` | Config, brand/DB/BC connection settings, `FormEnvironment`. **No longer identity** — see "Auth". **No longer `UatTester`** either — it keeps a permanent synonym pointing at the new home, for ACC Portal alone; see the row below |
...
| **Rocks_Portal_Form_UAT** (`UatTester`, `UatTesterPerDiem`) | `getUatFormPool()` | Who may test, and a per-tester per-diem override read only in UAT (migrations 139/140/141, moved from `Fast_Core` 2026-09-07). **One physical copy each, no dual-write, not in `MASTER_TABLES`.** A literal pool, never `getFormPool()`/`getAccPool()` — see "Parallel Production and UAT" below. `Fast_Core` keeps a permanent synonym for `UatTester` **alone**, since ACC Portal reads it there; `UatTesterPerDiem` gets none, since nothing outside this app names it |
...
- **UAT is visible only to configured testers.** `UatTester` (created by migration 063 in `Fast_Core`; moved to `Rocks_Portal_Form_UAT` by migrations 139/140 — `Fast_Core.dbo.UatTester` is now a synonym pointing there) holds the list, managed at **Settings → UAT Users** (`/settings/uat-users`, System Admin, API `/api/settings/uat-users`). Each tester carries a UAT `ManagerStaffId`, which must itself be an active tester — a chain cannot leak out of the test group. **A tester may be their own manager**, deliberately: that is how one person rehearses the whole submit-to-approve loop, and a UAT approval only ever approves test data. Removing a tester is a soft delete (`IsActive = 0`). Service: `src/lib/uat-tester/service.ts`.
...
Since migration 066 that is a hard constraint, not a preference: `auth()` no longer reads Fast_Core, it reads `TeamMember` in the form database. Everything on the path that decides *which* form database answers must therefore come from a pool this resolver does not pick — `getFormSwitchMap()` from `getCorePool()`, `getActiveUatTester()` from `getUatFormPool()` (moved 2026-09-07, migrations 139/140 — see below), identity from `getProductionFormPool()`. Each is a **literal**: none of the three consults the resolver, which is the actual invariant, and it is unchanged by where `UatTester` physically lives. **`FormEnvironment` must stay in Fast_Core**, for a different reason than the pool rule: `getFormSwitchMap()` is awaited unconditionally on every classified-form request, by every user, and holds `ProductionEnabled` — moving it would make production availability depend on the UAT database being up. **`UatTester` carried no such risk and has moved**, into `Rocks_Portal_Form_UAT`: `viewerIsTesting()` touches no database at all unless the UAT-mode cookie is on, so a production viewer never depends on the UAT database being reachable. What must stay true after the move is only that it is read through a pool the resolver never picks — `getUatFormPool()`, **never** `getFormPool()`/`getAccPool()` (`getAccPool` **is** `getFormPool()`, `src/lib/acc/pool.ts:4`) and **never** `getProductionFormPool()`, which would resolve `Rocks_Portal_Form`, where the table does not exist. `@/lib/team-member/service` must never reach for `getFormPool()` either.
...
- ❌ **It did not remove the Fast_Core dependency.** `getCorePool()` still has more than forty call sites across a dozen modules. `getFormSwitchMap()` (`src/lib/form-environment/service.ts`) is one of them: it resolves *which* form database answers, so it must read a database that resolver never picks — see "Parallel Production and UAT". `getActiveUatTester()` (`src/lib/uat-tester/service.ts`) sits on the same resolution path but no longer needs `getCorePool()` to satisfy it: migrations 139/140 (2026-09-07) moved `UatTester` into `Rocks_Portal_Form_UAT`, and it now reads `getUatFormPool()` — a different literal the resolver equally never picks. The app still cannot serve a request without Fast_Core: `getFormSwitchMap()` alone is awaited unconditionally on every classified-form request.
...
- **In UAT, a tester's per diem can come from their own configured rate instead
  ...
  `UatTesterPerDiem` (created by migration **138** in `Fast_Core`; moved to
  `Rocks_Portal_Form_UAT` by migrations 139/141, 2026-09-07 — `getUatFormPool()`,
  **never `getCorePool()`, `getFormPool()` or `getAccPool()`**, and no synonym is
  left behind, unlike `UatTester`'s — **one copy — not dual-written, not in
  `MASTER_TABLES`**) holds an effective-dated rate per StaffId, set at
  Settings → UAT Users. Design:
...
- **138 is `Fast_Core` only, and it must be applied before this code deploys — an unapplied 138 does not degrade, it breaks every AP-17 entry point for a UAT tester.** `138_core_uat_tester_per_diem.sql` creates `dbo.UatTesterPerDiem`, beside `UatTester` and `FormEnvironment` for the same reason those two lived there at the time: what a tester is paid must not depend on which form database answered. (`UatTester` has since moved into `Rocks_Portal_Form_UAT` via migrations 139/140, 2026-09-07 — see the 139/140/141 bullet below; `FormEnvironment` has not, and could not — see "Parallel Production and UAT" above.) It refuses any database other than `Fast_Core`. The read is reached through `withUatOverrides` (`src/lib/hr/employee-lookup.ts`), which sits inside `resolveEmployeeForActor` — and that function backs **five** paths, not one: the allowance-log route, the date-ranges route, the id-card consent POST, and AP-17's draft-save and submit in `request-service.ts`. `withUatOverrides` reads the table only when the resolved environment is UAT (`(await resolveFormEnvironment()) !== "UAT"` returns early otherwise), so the blast radius of a missing table is exactly "a UAT tester touches any of those five" — loud (`Invalid object name 'UatTesterPerDiem'`), not silent, and by design: see the migration's own header and the design spec's §9 for why degrading to "no override" was rejected — it would price a tester at their real HR allowance with no error, on a path that writes `AccRequest.TotalAmount`. (`id-card/previous` and its download route are *not* among the five: they were hardened to self-only, reading HR directly, by an earlier and unrelated security fix — see the corrected comment on `resolveEmployeeForActor` itself, which used to name them and no longer does.) **A second, independent seam reads the same table**: `getPerDiemEmployeeLog` / `getPerDiemEmployeeLogMap` (`allowance-log.ts`) → `uatPerDiemLogsByStaffIds`, which prices the trip itself rather than overriding a display value, and is called from **four** more sites that never go through `resolveEmployeeForActor` — the recompute (`perdiem-recompute.ts`), the submit's own pricing step and the report (`report-service.ts`) both name it directly, and the allowance-log route calls it a second time, for the entries themselves rather than the manager/allowance override `withUatOverrides` already gave it. **The recompute is the one that costs more than a 500.** `recomputeGroupPerDiem` runs inside the open transaction `rejectRequest`, `rejectByAdmin` and `cancelByRequester` each hold, so before 138 is applied, cancelling or rejecting a UAT trip whose group needs a re-price throws mid-transaction and rolls the whole action back — the status `UPDATE`, the approval close and the activity-log insert included, not only the per-diem read. The settings page's own GET/POST/PATCH degrade far more gently, since nothing there prices a trip — see `per-diem.ts`'s comment.
- **139/140/141 move `UatTester` and `UatTesterPerDiem` out of `Fast_Core`, and the order is 139 → 140 → deploy the code → 141 — not 139 → 140 → 141 → code, and not 139 → code → 140.** 139 (`Rocks_Portal_Form_UAT` only) creates both tables and copies their rows with ids preserved, reseeding each identity to the source's `IDENT_CURRENT` rather than `MAX(Id)` or the row count. 140 (`Fast_Core` only) drops `Fast_Core.dbo.UatTester` and replaces it with a permanent synonym pointing at the new home, for ACC Portal — it refuses unless 139's copy already exists and matches row-for-row. 141 (`Fast_Core` only) drops `Fast_Core.dbo.UatTesterPerDiem` outright, **with no synonym**, since nothing outside this app names that table. **141 alone must wait for the code deploy — that is the one ordering difference from 140.** A synonym is transparent to a build that predates it: after 140, the *old* code's `getCorePool()` reads of `UatTester` — `upsertUatTester`'s `MERGE` included — keep resolving correctly through it, which is what lets 140 run before the deploy with no window where Form Portal and ACC Portal disagree about the roster. `UatTesterPerDiem` gets no synonym, so there is nothing to be transparent through: running 141 before the deploy would hand the still-running old build (`getCorePool()` reads inside `withUatOverrides` and `allowance-log.ts`) an `Invalid object name` on AP-17's UAT pricing path — including inside the open transaction `rejectRequest`/`rejectByAdmin`/`cancelByRequester` hold for `recomputeGroupPerDiem`, which would roll the whole cancellation back. The other two-step order, 139 → code → 140, opens a different hole: Form Portal would write the new roster while ACC Portal still read `Fast_Core`'s now-frozen copy, with nothing erroring — a real disagreement about who may test. Run `npm run check:alignment` after each step; it must stay at **27**, since neither table is dual-written or in `MASTER_TABLES`, so the count does not move at any point. **`npm run check:uat-tester-home`** (`scripts/checks/verify-uat-tester-move.ts`) is the post-move check, run after 141: it opens `getUatFormPool()` — the pool the app itself resolves through, not a literal database name — confirms both tables are real tables there, confirms `Fast_Core.dbo.UatTester`'s synonym `base_object_name` names that same database, compares a count read through the synonym against a direct count in one round trip, and confirms `Fast_Core` holds no `UatTesterPerDiem` object of any kind. It is what catches the `MSSQL_FORM_UAT_DATABASE`/migration-140 env-drift hazard, the same shape CLAUDE.md already records for `MSSQL_FORM_DATABASE`/100 and `MSSQL_ERP_DATA_DATABASE`/102.
