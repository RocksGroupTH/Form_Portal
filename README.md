...
| `Fast_Data` | Nothing this app reads any more. `TravelProvince` moved to `Rocks_Portal_Form` (migrations 104/105), completing the same move already made for the five Business Central sync tables (→ `Rocks_ERP_Data`, 101/102) and the department map (→ `Fast_Core`, 099/100). The database still physically holds Rocks Fast's Intelligence tables, which this app never touches, plus a synonym per moved table for the Rocks Fast and ACC Portal siblings. |
