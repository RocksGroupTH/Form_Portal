...
| `npm run check:alignment` | Assert the dual-written master tables match across Production and UAT |
| `npm run check:manager-drift` | Report requests whose manager HR names today is not the one stamped at submit |
...
| `Rocks_Portal_Form_UAT` | The UAT twin, served to configured testers, plus `TesterPerDiem` — a per-tester per-diem override read only in UAT (migrations 139/140, moved from `Fast_Core`, no synonym; renamed off `UatTesterPerDiem` by 142) |
