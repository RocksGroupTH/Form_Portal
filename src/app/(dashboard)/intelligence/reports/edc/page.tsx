...
const columns: ColumnDef<Row, unknown>[] = [
  ...
  { accessorKey: "cardType", header: "Card Type", enableGrouping: true },
  { accessorKey: "edcSerial", header: "EDC Serial", enableGrouping: true },
  { accessorKey: "creditCardId", header: "Card ID", enableGrouping: false },
  {
    accessorKey: "amount",
    header: "Amount",
...
export default function EdcSettlementPage() {
  return <Suspense><EdcContent /></Suspense>;
}

function EdcContent() {
  const searchParams = useSearchParams();
  const [brand] = useState(searchParams.get("brand") ?? "UNO");
  const { from, to, branch, setFrom, setTo, setBranch } = useReportFilters("edc");

  const params = new URLSearchParams({ brand, from, to });
  if (branch) params.set("branch", branch);

  const { data, isLoading } = useSWR<{ ok: boolean; data: Row[] }>(
    `/api/intelligence/reports/edc?${params}`,
    fetcher,
  );
  ...
  const kpis = useMemo(() => {
    ...
    const cardTypes = new Set(rows.map((r) => r.cardType)).size;
    return [
      { label: "Total Amount", value: fmtBaht(totalAmount), color: "#2563eb" },
      { label: "Transactions", value: fmtNum(transactions) },
      { label: "Card Types", value: fmtNum(cardTypes) },
    ];
  }, [rows]);
    ...
    <PageContainer className="py-3 px-3 sm:px-0 flex-1 flex flex-col min-h-0" maxWidth="2k">
      ...
      <PageHeaderCard className="flex items-center gap-2 flex-wrap mb-3">
        ...
        <span className="relative group">
          ...
          <span className="absolute left-1/2 -translate-x-1/2 top-full mt-1 px-2.5 py-1.5 rounded-lg text-[10px] font-normal whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-40" style={{ background: "var(--bg-dropdown)", color: "var(--text-muted)", border: "1px solid var(--border-card)", boxShadow: "var(--shadow-sm)" }}>
            Credit card transactions &mdash; card type, EDC serial, settlement &middot; Source: Foodstory POS
          </span>
        </span>
        <span
      ...
      {isLoading ? (
        <ReportLoading />
          ...
          <DataTable data={rows} columns={columns} groupBy={["branchName"]} reportName="EDC" emptyState={<ReportEmptyState branch={branchLabel} from={from} to={to} />} />
      )}
    </PageContainer>
  );
}
