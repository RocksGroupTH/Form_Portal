...
const columns: ColumnDef<Row, unknown>[] = [
  ...
  { accessorKey: "dateTime", header: "Time", enableGrouping: false },
  ...
  { accessorKey: "branchName", header: "Branch", enableGrouping: true },
  {
    ...
    cell: ({ getValue }) => {
      ...
      return v.length > 12 ? <span title={v}>{v.slice(0, 12)}...</span> : v;
    },
  },
  { accessorKey: "voucherType", header: "Voucher Type", enableGrouping: true },
  {
    accessorKey: "voucherValue",
    header: "Value",
    cell: ({ getValue }) => fmtBaht(getValue() as number),
    aggregatedCell: ({ getValue }) => <strong>{fmtBaht(getValue() as number)}</strong>,
    aggregationFn: "sum",
...
function PromotionContent() {
  ...
  const kpis = useMemo(() => {
    ...
    const discountAmount = rows.reduce((s, r) => s + (r.discountAmount ?? 0), 0);
    const vouchers = new Set(rows.map((r) => r.voucherCode)).size;
    const receipts = new Set(rows.map((r) => r.receiptNo)).size;
    return [
      { label: "Total Amount", value: fmtBaht(totalAmount), color: "#2563eb" },
      { label: "Discount", value: fmtBaht(discountAmount), color: "#dc2626" },
      { label: "Vouchers Used", value: fmtNum(vouchers) },
    ...
    <PageContainer className="py-3 px-3 sm:px-0 flex-1 flex flex-col min-h-0" maxWidth="2k">
      ...
      <PageHeaderCard className="flex items-center gap-2 flex-wrap mb-3">
        ...
        <span className="relative group">
          ...
          <span className="absolute left-1/2 -translate-x-1/2 top-full mt-1 px-2.5 py-1.5 rounded-lg text-[10px] font-normal whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-40" style={{ background: "var(--bg-dropdown)", color: "var(--text-muted)", border: "1px solid var(--border-card)", boxShadow: "var(--shadow-sm)" }}>
            Voucher usage, discount amounts, promo codes, redemption rate &middot; Source: Foodstory POS
          </span>
        </span>
        <span
      ...
      {isLoading ? (
        <ReportLoading />
          ...
          <DataTable data={rows} columns={columns} groupBy={["branchName"]} reportName="Voucher" emptyState={<ReportEmptyState branch={branch} from={from} to={to} />} />
      )}
    </PageContainer>
  );
}
