...
const columns: ColumnDef<Row, unknown>[] = [
  ...
  { accessorKey: "tenderGroup", header: "Tender Group", enableGrouping: true },
  { accessorKey: "tenderDetail", header: "Tender Detail", enableGrouping: true },
  {
    accessorKey: "bills",
    header: "Bills",
...
function TenderContent() {
  ...
  const kpis = useMemo(() => {
    ...
    const tenderTypes = new Set(rows.map((r) => r.tenderGroup)).size;
    return [
      { label: "Revenue", value: fmtBaht(revenue), color: "#2563eb" },
      { label: "Bills", value: fmtNum(bills) },
      { label: "Tender Types", value: fmtNum(tenderTypes) },
    ];
  }, [rows]);
    ...
    <PageContainer className="py-3 px-3 sm:px-0 flex-1 flex flex-col min-h-0" maxWidth="2k">
      ...
      <PageHeaderCard className="flex items-center gap-2 flex-wrap mb-3">
        ...
        <span className="relative group">
          ...
          <span className="absolute left-1/2 -translate-x-1/2 top-full mt-1 px-2.5 py-1.5 rounded-lg text-[10px] font-normal whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-40" style={{ background: "var(--bg-dropdown)", color: "var(--text-muted)", border: "1px solid var(--border-card)", boxShadow: "var(--shadow-sm)" }}>
            Payment breakdown — QR, Grab Pay, Cash, Credit Card, LINE Pay &middot; Source: Foodstory POS
          </span>
        </span>
      ...
      {isLoading ? (
        <ReportLoading />
          ...
          <DataTable data={rows} columns={columns} groupBy={["tenderGroup"]} reportName="Tender" emptyState={<ReportEmptyState branch={branchLabel} from={from} to={to} />} />
      )}
    </PageContainer>
  );
}
