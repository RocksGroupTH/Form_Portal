...
function VatContent() {
  ...
  const kpis = useMemo(() => {
    ...
    const grossSales = rows.reduce((s, r) => s + (r.grossSales ?? 0), 0);
    const netSales = rows.reduce((s, r) => s + (r.netSales ?? 0), 0);
    const vat = rows.reduce((s, r) => s + (r.vatAmount ?? 0), 0);
    const bills = rows.reduce((s, r) => s + (r.bills ?? 0), 0);
    return [
      { label: "Gross Sales", value: fmtBaht(grossSales), color: "#2563eb" },
      { label: "Net Sales", value: fmtBaht(netSales), color: "#2563eb" },
      { label: "VAT", value: fmtBaht(vat), color: "#dc2626" },
    ...
    <PageContainer className="py-3 px-3 sm:px-0 flex-1 flex flex-col min-h-0" maxWidth="2k">
      ...
      <PageHeaderCard className="flex items-center gap-2 flex-wrap mb-3">
        ...
        <span className="relative group">
          ...
          <span className="absolute left-1/2 -translate-x-1/2 top-full mt-1 px-2.5 py-1.5 rounded-lg text-[10px] font-normal whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-40" style={{ background: "var(--bg-dropdown)", color: "var(--text-muted)", border: "1px solid var(--border-card)", boxShadow: "var(--shadow-sm)" }}>
            Tax summary — net sales, VAT amount, by date and branch &middot; Source: Foodstory POS
          </span>
        </span>
      ...
      {isLoading ? (
        <ReportLoading />
          ...
          <DataTable data={rows} columns={columns} groupBy={["branchName"]} reportName="VAT" emptyState={<ReportEmptyState branch={branchLabel} from={from} to={to} />} />
      )}
    </PageContainer>
  );
}
