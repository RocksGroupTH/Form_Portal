...
const columns: ColumnDef<Row, unknown>[] = [
  ...
  },
  {
    ...
    aggregatedCell: ({ row }) => {
      ...
      const totalAmt = leaves.reduce((s, r) => s + ((r.original as Row).totalPrice ?? 0), 0);
  ...
  { accessorKey: "voidBy", header: "Voided By", enableGrouping: true },
  {
    accessorKey: "voidReason",
    header: "Reason",
    cell: ({ getValue }) => <span style={{ color: "#ef4444" }}>{getValue() as string}</span>,
    enableGrouping: true,
  },
  { accessorKey: "voidStaff", header: "Void Staff", enableGrouping: true },
];

export default function VoidReportPage() {
  return <Suspense><VoidContent /></Suspense>;
}

function VoidContent() {
  const searchParams = useSearchParams();
  const [brand] = useState(searchParams.get("brand") ?? "UNO");
  const { from, to, branch, setFrom, setTo, setBranch } = useReportFilters("void");

  const params = new URLSearchParams({ brand, from, to });
  if (branch) params.set("branch", branch);

  const { data, isLoading } = useSWR<{ ok: boolean; data: Row[] }>(
    `/api/intelligence/reports/void?${params}`,
    fetcher,
  );
  ...
  const kpis = useMemo(() => {
    ...
    const voidAmount = rows.reduce((s, r) => s + (r.totalPrice ?? 0), 0);
    const items = rows.reduce((s, r) => s + (r.quantity ?? 0), 0);
    const receipts = new Set(rows.map((r) => r.receiptNo)).size;
    const reasons = new Set(rows.map((r) => r.voidReason)).size;
    return [
      { label: "Void Amount", value: fmtBaht(voidAmount), color: "#dc2626" },
      { label: "Items", value: fmtNum(items) },
      { label: "Receipts", value: fmtNum(receipts) },
      { label: "Void Reasons", value: fmtNum(reasons) },
    ];
  }, [rows]);
    ...
    <PageContainer className="py-3 px-3 sm:px-0 flex-1 flex flex-col min-h-0" maxWidth="2k">
      ...
      <PageHeaderCard className="flex items-center gap-2 flex-wrap mb-3">
        ...
        <span className="relative group">
          ...
          <span className="absolute left-1/2 -translate-x-1/2 top-full mt-1 px-2.5 py-1.5 rounded-lg text-[10px] font-normal whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-40" style={{ background: "var(--bg-dropdown)", color: "var(--text-muted)", border: "1px solid var(--border-card)", boxShadow: "var(--shadow-sm)" }}>
            Voided items &mdash; who voided, reason, amount lost, by branch &middot; Source: Foodstory POS
          </span>
        </span>
        <span
      ...
      {isLoading ? (
        <ReportLoading />
          ...
          <DataTable data={rows} columns={columns} groupBy={["branchName"]} reportName="Void" emptyState={<ReportEmptyState branch={branchLabel} from={from} to={to} />} />
      )}
    </PageContainer>
  );
}
