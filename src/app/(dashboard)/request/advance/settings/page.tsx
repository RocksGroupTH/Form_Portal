...
const TAB_META: Record<TabKey, { label: string; icon: React.ReactNode }> = {
  ...
  // `@/lib/adv/settings-tabs` excludes `advanceMessages` from
  ...
  // `canAdvanceMessage`, which reads `AccAdvClrAccess.CanAdvanceMessage`, a
...
function useVisibleTabs() {
  const { data } = useSWR<{
    ...
    data?: {
      ...
      /** `AccAdvClrAccess.CanAdvanceMessage` (migration 166) — a column, not a settingsTabs entry. */
  ...
  // `advanceMessages` is a SEPARATE arm: `settingsTabs` never carries it and
  // never must — the grant is a COLUMN, not a `TabKey` row. See
  // `@/lib/acc/message-grant`.
  return TABS.filter(
    (t) => d.settingsTabs?.[t.key] || (t.key === "advanceMessages" && d.canAdvanceMessage),
  );
}
