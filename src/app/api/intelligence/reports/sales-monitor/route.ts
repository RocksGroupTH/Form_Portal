...
/* ── GET /api/intelligence/reports/sales-monitor ── */
  ...
  try {
    ...
    const result = await request.query(`
      ...
      WHERE b.void_flag != '1' AND b.is_revenue = '1'
        ...
        ${branchFilter}
      GROUP BY b.IngestDate, m.branch_name, b.order_type, b.channel
      ORDER BY b.IngestDate DESC, SUM(b.discounted_price_num) DESC
    `);

    return NextResponse.json({ ok: true, data: result.recordset });
  } catch (err) {
    console.error("[api/intelligence/reports/sales-monitor] GET", err);
