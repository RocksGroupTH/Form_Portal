...
import { returnCurrentStep } from "@/lib/adv/advance-approval-engine";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!stepType) return NextResponse.json({ ok: false, error: "ไม่อยู่ในขั้นที่ส่งกลับได้" }, { status: 400 });
  ...
  try {
    ...
    await returnCurrentStep(id, actionActor, body.comment);
