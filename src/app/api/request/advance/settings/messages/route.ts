...
import { requireAdvanceMessageAccess } from "@/lib/adv/require-adv-clr-message-access";
...
/**
 * AP-2's notice copy — the box at the top of the advance form.
 *
 ...
 *
 * **Gated on `AccAdvClrAccess.CanAdvanceMessage` (migration 166), a COLUMN, not
 * a `TabKey` grant — and that is storage, not risk.** A message grants
 ...
 * side. `CanAdvanceMessage` is a column that saver's explicit column list
 * never names, so it survives. See `@/lib/acc/message-grant` and
 * `requireAdvanceMessageAccess`'s own docblock. `advanceMessages` therefore
 * stays out of `GRANTABLE_ADV_CLR_TABS` — that list is `TabKey` grants only —
 * even though this route is no longer admin-only.
 *
 ...
 */
const FORM_CODE = "AP-2";

export async function GET() {
  const session = await requireAdvanceMessageAccess();
  ...
  try {
    ...
    console.error("[api/request/advance/settings/messages] GET", err);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error("[api/request/advance/settings/messages] POST", err);
    return NextResponse.json({ ok: false, error: "Internal server error" }, { status: 500 });
  }
}
