...
import { saveClrBatch } from "@/lib/clr/clear-advance-interface-config-service";
...
export async function POST(req: NextRequest) {
  ...
  if (session instanceof Response) return session;
  try {
    const body = (await req.json().catch(() => ({}))) as { brandCode?: string; journalBatchName?: string };
    const brandCode = (body.brandCode ?? "").trim();
    if (!brandCode) return NextResponse.json({ ok: false, error: "กรุณาเลือกแบรนด์" }, { status: 400 });
    await saveClrBatch(brandCode, (body.journalBatchName ?? "").trim(), Number(session.user.id));
