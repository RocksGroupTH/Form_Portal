...
import { requireClearMessageAccess } from "@/lib/adv/require-adv-clr-message-access";
...
/**
 * AP-3's notice copy — the box at the top of the clear-advance form.
 *
 * **`FORM_CODE` is a literal and is never read from the body.** Which form a
 * route governs is a property of the route, the same rule `requireAdvClrSettingsTab`
 * states for its tab key; a posted form code would let this route rewrite
 * another form's notice.
 *
 * **Gated on `AccAdvClrAccess.CanClearMessage` (migration 166), a COLUMN, not a
 * `TabKey` grant — and that is storage, not risk.** A message grants
 * nothing — no approval, no posting target, no read. But `AccAdvClrAccessTab`
 * is shared with the ACC Portal sibling, whose own save deletes every row for
 * an approver and re-inserts only the keys its list knows, so a `messages`
 * grant stored THERE would vanish on that app's next save with no error either
 * side. `CanClearMessage` is a column that saver's explicit column list never
 * names, so it survives. See `@/lib/acc/message-grant` and
 * `requireClearMessageAccess`'s own docblock. `clearMessages` therefore stays
 * out of `GRANTABLE_ADV_CLR_TABS` — that list is `TabKey` grants only — even
 * though this route is no longer admin-only.
 *
 ...
 */
const FORM_CODE = "AP-3";

export async function GET() {
  const session = await requireClearMessageAccess();
  ...
  try {
    ...
    console.error("[api/request/clear-advance/settings/messages] GET", err);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error("[api/request/clear-advance/settings/messages] POST", err);
    return NextResponse.json({ ok: false, error: "Internal server error" }, { status: 500 });
  }
}
