...
function accountRules(candidates: ExpenseAccount[]): string {
  if (candidates.length === 0) {
    // Only when the brand is unknown or its chart is empty — not the day-one
    // case any more, which is why the list is no longer history-only.
    return ["", "- accountNo: ให้ตอบ null เสมอ (ยังไม่มีรายการบัญชีให้เลือก)"].join("\n");
  }
  return [
    "",
    "- accountNo: **ต้องเลือกเสมอถ้าพอจะจับคู่ได้** โดยดูจาก description เป็นหลัก",
    "  ว่าค่าใช้จ่ายบรรทัดนี้เข้าข่ายบัญชีไหนมากที่สุด",
    "  เลือก **จากรายการข้างล่างนี้เท่านั้น** ตอบเป็นเลขบัญชีเป๊ะ ๆ ห้ามแต่งเลขขึ้นเอง",
    "  รายการเรียงโดยเอาบัญชีที่บริษัทนี้เคยใช้จริงขึ้นก่อน",
    "  ตอบ null เฉพาะเมื่อไม่มีบัญชีไหนใกล้เคียงเลย — การเดาบัญชีแย่กว่าการเว้นว่าง",
    "",
...
const COMMON_RULES = [
  ...
  "- description: **ซื้ออะไร** สั้น ๆ เป็นภาษาไทย",
  "  ห้ามใส่ชื่อบริษัท ชื่อร้าน หรือชื่อผู้ขาย — มีช่อง vendorName แยกไว้แล้ว",
  "  เขียนให้บอกได้ว่าเป็นค่าใช้จ่ายประเภทไหน เช่น 'ค่าทำป้ายไฟและสติกเกอร์ พร้อมติดตั้ง'",
