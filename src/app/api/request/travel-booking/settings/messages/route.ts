...
import { requireBookingMessageAccess } from "@/lib/acc/travel-booking/require-booking-message-access";
 ...
 *
 * **Gated on `AccBookingApprover.CanMessage` (migration 166), a COLUMN, not a
 * `TabKey` grant — and that is storage, not risk.** A message grants nothing —
 * no approval, no posting target, no read. But `AccBookingApproverTab` is
 ...
 * survives. See `@/lib/acc/message-grant` and `requireBookingMessageAccess`'s
 * own docblock. `messages` therefore stays out of `GRANTABLE_BOOKING_TABS` —
 ...
 */
const FORM_CODE = "AP-17";

export async function GET() {
  const session = await requireBookingMessageAccess();
  ...
  try {
    ...
    console.error("[api/request/travel-booking/settings/messages] GET", err);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error("[api/request/travel-booking/settings/messages] POST", err);
    return NextResponse.json({ ok: false, error: "Internal server error" }, { status: 500 });
  }
}
