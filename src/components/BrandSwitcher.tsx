...
export function BrandSwitcher({ compact = false }: BrandSwitcherProps) {
  ...
  /**
   * **Nothing to switch to, so nothing is shown** (user, 2026-09-15:
   * "ซ่อน brand ไว้ก่อน").
   *
   * One brand on offer — measured that day, only `ROCKS` is enabled at
   * Settings → Brand Configuration — made "Switch Brand" a dialog whose single
   * tile was already current, a click that could only close what it opened. It
   * was briefly a static label instead; hiding it outright is what was asked
   * for, and reads better: a chip naming the only company there is says nothing
   * a reader did not already know.
   *
   * **Hiding the chip does NOT stop the brand being chosen.** `BrandGate` still
   * adopts the sole brand and writes the `rocks-fast-brand` cookie, which is
   * what the server reads to resolve ERP and Business Central context — the
   * navbar was only ever the display.
   *
   * Keyed on the list, not on the brand's code: re-enable a second brand and
   * the switcher comes back by itself, because then there is a real choice to
   * make. Same rule `BrandGate` uses, for the same reason.
   */
