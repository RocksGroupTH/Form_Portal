 ...
 * card **or a passport** — the two documents the field invites, and the only
 * two the route accepts. The passport half was the user's ruling of
 * 2026-09-22; see the route's docblock for why it had to be a ruling and not a
 * copy fix.
 *
 * **`looksLikeThaiIdCard` is now a narrower name than what it asks**, and it
 * was left alone on purpose: `isIdCard`, `IdCardUpload`, `id-card-access.ts`
 * and the route path itself all share that vocabulary, so renaming this one
 * symbol would make the set less consistent rather than more. Read the name as
 * "the AP-17 identity-document check", not as a claim about what passes.
 *
 ...
 * the cost stated and accepted: the document is required to submit, so while
 ...
 * heuristic produced, and the point of the rebuild was to stop that. Widening
 * the *accepted set* on 2026-09-22 did not touch this: a passport is now one
 * of the two documents that can pass, and everything else refuses exactly as
 * it did.
 *
 * `ok: false` covers both "this is not an accepted document" and "no verdict
 * could be obtained", and `reason` is always set to something the requester
 * can act on:
...
function unavailableReason(status: number): string {
  if (status === 429) return "ตรวจรูปเอกสารถี่เกินไป กรุณารอสักครู่แล้วลองใหม่";
  if (status === 503) return "ระบบตรวจรูปเอกสารยังไม่พร้อมใช้งาน — กรุณาแจ้งฝ่าย IT";
  return "ตรวจสอบรูปเอกสารไม่สำเร็จ กรุณาลองใหม่อีกครั้ง — ถ้ายังไม่ได้ กรุณาแจ้งฝ่าย IT";
}
...
export async function looksLikeThaiIdCard(file: File): Promise<IdCardCheck> {
  ...
  try {
    ...
    return json.data?.isIdCard === true
      ? { ok: true }
      : { ok: false, reason: json.data?.reason ?? "รูปนี้ไม่ใช่บัตรประจำตัวประชาชนหรือพาสปอร์ต" };
  } catch {
