...
import { onBehalfNotifyList, isOnBehalf } from "./on-behalf";
...
test("the comparison ignores case and surrounding space, on BOTH sides", () => {
  ...
  assert.deepEqual(
    onBehalfNotifyList([], { requesterEmail: " " + FOR.toUpperCase() + " ", filerEmail: FOR }),
    [],
  );
});
...
test("the filer is added to a trigger that only named the requester", () => {
  assert.deepEqual(
    onBehalfNotifyList([FOR], { requesterEmail: FOR, filerEmail: BY }),
    [FOR, BY],
  );
});
...
test("on submit, the person it was filed for is told as well", () => {
  ...
  assert.deepEqual(
    onBehalfNotifyList([APPROVER], { requesterEmail: FOR, filerEmail: BY }, { alsoRequester: true }),
    [APPROVER, BY, FOR],
  );
});
...
test("a self-filed request gains nobody, on any trigger", () => {
  assert.deepEqual(
    onBehalfNotifyList([APPROVER], { requesterEmail: FOR, filerEmail: FOR }, { alsoRequester: true }),
    [APPROVER],
  );
  assert.deepEqual(
    onBehalfNotifyList([FOR], { requesterEmail: FOR, filerEmail: FOR }),
    [FOR],
  );
});
...
test("alsoRequester is off by default, so only submit tells the requester", () => {
  assert.deepEqual(
    onBehalfNotifyList([APPROVER], { requesterEmail: FOR, filerEmail: BY }),
    [APPROVER, BY],
  );
});
...
test("a filer already on the roster is not mailed twice", () => {
  ...
  assert.deepEqual(
    onBehalfNotifyList([BY, FOR], { requesterEmail: FOR, filerEmail: BY }),
    [BY, FOR],
  );
});
...
test("duplicates and blanks in the caller's own list are dropped", () => {
  assert.deepEqual(
    onBehalfNotifyList([APPROVER, "", null, " " + APPROVER.toUpperCase() + " ", undefined], { requesterEmail: FOR, filerEmail: BY }),
    [APPROVER, BY],
  );
});
...
test("the first spelling wins, so the queue reads the way the caller wrote it", () => {
  assert.deepEqual(
    onBehalfNotifyList([APPROVER.toUpperCase()], { requesterEmail: FOR, filerEmail: APPROVER }),
...
test("a request with no requester email still reaches its filer", () => {
  ...
  assert.deepEqual(onBehalfNotifyList([], { requesterEmail: null, filerEmail: BY }), [BY]);
});

test("the input list is not mutated", () => {
  const base = [APPROVER];
  onBehalfNotifyList(base, { requesterEmail: FOR, filerEmail: BY }, { alsoRequester: true });
