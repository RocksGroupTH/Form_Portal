...
import type { AccApproverRow, AccVehicle, AccSameDayBrandRow } from "@/features/accounting/types";
...
export async function listVehicles(activeOnly = false): Promise<AccVehicle[]> {
  ...
  return r.recordset.map((x: Record<string, unknown>) => ({
    id: x.Id as number, name: x.Name as string,
    ratePerKm: (x.RatePerKm as number) ?? null,
    isManualEntry: !!x.IsManualEntry, isActive: !!x.IsActive, sortOrder: x.SortOrder as number,
    icon: (x.Icon as string) ?? null,
  }));
}

export async function upsertVehicle(
  v: Partial<AccVehicle> & { name: string; isManualEntry: boolean }, userId: number,
): Promise<void> {
  ...
  try {
    for (let i = 0; i < orderedIds.length; i++) {
      await tx.request()
        ...
        .query(`UPDATE [dbo].[AccVehicle] SET SortOrder=@sort, UpdatedAt=SYSDATETIME() WHERE Id=@id`);
    }
    await tx.commit();
...
/* ---- Approvers ---- */
export async function listApprovers(activeOnly = false): Promise<AccApproverRow[]> {
  const pool = await getAccPool();
  const r = await pool.request().query(`
  ...
  const rows = r.recordset.map((x: Record<string, unknown>) => ({
    id: x.Id as number, staffId: (x.StaffId as number) ?? null, email: x.Email as string,
    displayName: (x.DisplayName as string) ?? null, isActive: !!x.IsActive,
    photoUrl: (x.PhotoUrl as string) ?? null,
  }));
  const brandMap = await loadInterfaceBrandsByApproverIds(rows.map((row) => row.id));
  return rows.map((row) => ({
    ...row,
...
export async function getApproverIdByEmail(email: string): Promise<number | null> {
  const pool = await getAccPool();
...
export async function upsertApprover(
  a: { id?: number; staffId?: number | null; email?: string; displayName?: string | null; isActive?: boolean; photoUrl?: string | null },
  userId: number,
): Promise<void> {
  const pool = await getAccPool();
...
/* ---- Form brands ---- */
export async function listFormBrands(formCode: string): Promise<{ id: number; brandCode: string; isActive: boolean; sortOrder: number }[]> {
  const pool = await getAccPool();
  ...
  `);
  return r.recordset.map((x: Record<string, unknown>) => ({
    id: x.Id as number, brandCode: x.BrandCode as string, isActive: !!x.IsActive, sortOrder: x.SortOrder as number,
  }));
}

export async function setFormBrands(formCode: string, brandCodes: string[]): Promise<void> {
  const pool = await getAccPool();
  const tx = pool.transaction();
  await tx.begin();
  try {
    await tx.request().input("form", sql.NVarChar, formCode)
      .query(`UPDATE [dbo].[AccFormBrand] SET IsActive = 0 WHERE FormCode = @form`);
    for (let i = 0; i < brandCodes.length; i++) {
      await tx.request()
        .input("form", sql.NVarChar, formCode)
        .input("brand", sql.NVarChar, brandCodes[i])
        .input("sort", sql.Int, i)
        .query(`MERGE [dbo].[AccFormBrand] AS t
...
export async function getSetting(key: string): Promise<string | null> {
  ...
  const r = await pool.request().input("key", sql.NVarChar, key)
    .query(`SELECT SettingValue FROM [dbo].[AccSetting] WHERE SettingKey = @key`);
...
export async function setSetting(key: string, value: string | null, userId: number): Promise<void> {
  ...
  await pool.request()
    ...
    .input("user", sql.Int, userId || null)
    .query(`MERGE [dbo].[AccSetting] AS t USING (SELECT @key AS SettingKey) AS s
...
export async function upsertSameDayBrandStaff(
  a: { id?: number; staffId?: number | null; email?: string | null; displayName?: string | null; isActive?: boolean },
  userId: number,
): Promise<void> {
  const pool = await getAccPool();
  const req = pool.request()
  ...
  if (a.id) {
    ...
    if (a.staffId == null) throw new Error("staffId is required to add a same-day-brand entry");
